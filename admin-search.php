<?php
  ...
  if ( isset($_POST['search']) || isset($_POST['selected']) ) {
    ...
    if($value == ""){
      $sql = "SELECT * FROM users INNER JOIN user_job_info ON users.id = user_job_info.users_id ORDER BY lastname ASC LIMIT " . $offset . ", " . $items_per_page;
      $result = mysqli_query($db, $sql);

      $sql2 = "SELECT * FROM users INNER JOIN user_job_info ON users.id = user_job_info.users_id";
      ...
      //sql for search
      $sql = "SELECT * FROM users INNER JOIN user_job_info ON users.id = user_job_info.users_id CONCAT_WS('', ".$test3.") LIKE '%$value%' LIMIT ". $offset . ", " . $items_per_page;
      $result = mysqli_query($db, $sql);
